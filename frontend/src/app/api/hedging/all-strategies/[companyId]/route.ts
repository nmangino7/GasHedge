...
import { compareAllStrategies, recommendEtfOptionsStrategies } from "@/lib/hedging-engine";
...
export async function GET(
  ...
  try {
    ...
    const [fuelPrice, etfPricesFallback, liveQuotes] = await Promise.all([
      getCurrentPrice(fuelType, company.padd_region),
      getAllETFPrices(),
      getMultipleQuotes(["UGA", "USO", "BNO", "UNL"]).catch(() => ({})),
    ]);

    const etfPrices: Record<string, number> = { ...etfPricesFallback };
    for (const [k, v] of Object.entries(liveQuotes)) {
      if (v?.price && v.price > 0) etfPrices[k] = v.price;
    }

    const baseResult = compareAllStrategies(monthlyGallons, fuelType, fuelPrice || 3.5, etfPrices, hedgeRatio);
    const optionsStrategies = recommendEtfOptionsStrategies(monthlyGallons, fuelType, fuelPrice || 3.5, etfPrices, hedgeRatio);
